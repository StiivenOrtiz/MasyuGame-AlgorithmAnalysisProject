...
# the cell clicked
def check_cell_click(mouse_pos, cell_size, margin_size, N_CELLS):
    """
...
def is_line_exits(cell1, cell2, drawn_lines):
    """
    @param cell1: tuple (row, col) with the position of the first cell
    @param cell2: tuple (row, col) with the position of the second cell
...
def delete_lines_like(cell1, cell2, drawn_lines):
    """
    @param cell1: tuple (row, col) with the position of the first cell
    @param cell2: tuple (row, col) with the position of the second cell
    @param drawn_lines: list with the lines drawn
...
def delete_diagonal_lines(drawn_lines):
    """
    @param drawn_lines: list with the lines drawn
...
def delete_non_adjacent_lines(drawn_lines):
    """
    @param drawn_lines: list with the lines drawn
...
def detects_lines(
    ...
    if mouse_buttons[0]:  # The left mouse button is pressed
        cell_clicked = check_cell_click(
            mouse_pos, CELL_SIZE, MARGIN_SIZE, N_CELLS)
        if cell_clicked and cell_clicked != prev_cell_clicked:
            if is_adjacent(prev_cell_clicked, cell_clicked):
                if (is_line_exits(prev_cell_clicked, cell_clicked, drawn_lines)):
                    drawn_lines = delete_lines_like(
                        prev_cell_clicked, cell_clicked, drawn_lines)
                else:
                    drawn_lines.append(prev_cell_clicked)
                    drawn_lines.append(cell_clicked)
                drawn_lines = delete_diagonal_lines(drawn_lines)
                drawn_lines = delete_non_adjacent_lines(drawn_lines)
